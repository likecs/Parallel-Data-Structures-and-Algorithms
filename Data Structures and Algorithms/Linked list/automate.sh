...
while [ $counter -gt 0 ]; do
	...
	timeout 10s ./Optimistic\ Synchronisation/linked_list < ./Generators/inp$2/$i'.in' > ./Optimistic\ Synchronisation/out$2/$i'.out'
	if [[ $? -ne 0 ]]; then
		echo error
	else
	...
	timeout 5s ./Lazy\ Synchronisation/linked_list < ./Generators/inp$2/$i'.in' > ./Lazy\ Synchronisation/out$2/$i'.out'
	if [[ $? -ne 0 ]]; then
		echo error
	else
